...
class GitHubApi:
        ...
        try:
            ...
            if toggle_name == "private_vulnerability_reporting" and exc.status == 422:
                raise ApiError(
                    ...
                    exc.path,
                    exc.status,
                    "private vulnerability reporting is unavailable for this repository",
                ) from exc
            if exc.status == 404:
                return False
            raise
    ...
    def list_rulesets(self, owner: str, repo: str) -> list[dict[str, Any]]:
        return self._request_json(
            "GET", f"/repos/{owner}/{repo}/rulesets?includes_parents=false&per_page=100"
        )

    def get_ruleset(self, owner: str, repo: str, ruleset_id: int) -> dict[str, Any]:
        return self._request_json(
            "GET", f"/repos/{owner}/{repo}/rulesets/{ruleset_id}?includes_parents=false"
        )

    def create_ruleset(self, owner: str, repo: str, payload: dict[str, Any]) -> dict[str, Any]:
        return self._request_json(
            "POST", f"/repos/{owner}/{repo}/rulesets", payload, expected_statuses={201}
        )
    ...
    def create_pages_site(self, owner: str, repo: str, payload: dict[str, Any]) -> dict[str, Any]:
        return self._request_json(
            "POST", f"/repos/{owner}/{repo}/pages", payload, expected_statuses={201}
        )
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    validate_top_level_keys(
        data, {"repository", "security", "pages", "rulesets", "unsupported"}, "manifest"
    )
...
def build_plan(
    api: GitHubApi, owner: str, repo: str, config: dict[str, Any], mode: str, hostname: str
) -> PlanResult:
    changes: list[PlannedChange] = []
    unsupported: list[UnsupportedSetting] = []
    warnings: list[str] = []
    ...
    if advanced_security is not None:
        if not is_private_repo:
            ...
            current_status = (
                (current_repo.get("security_and_analysis") or {}).get("advanced_security") or {}
            ).get("status")
    ...
    ruleset_details = get_managed_rulesets(
        api, owner, repo, [desired_branch_ruleset, desired_tag_ruleset]
    )
    changes.extend(
...
def coerce_dependency_graph_setting(security_config: dict[str, Any]) -> bool | None:
    ...
    if (
        ...
        and vulnerability_alerts is not None
        and dependency_graph != vulnerability_alerts
    ):
        raise ConfigError(
...
def plan_pages_changes(
    api: GitHubApi, owner: str, repo: str, pages_config: dict[str, Any]
) -> list[PlannedChange]:
    desired = build_pages_state(pages_config)
    current = api.get_pages_site(owner, repo)

    if current is None:
        update_payload = {key: value for key, value in desired.items() if key not in {"build_type"}}
        return [
            PlannedChange(
                key="pages.site",
...
def extract_unsupported_settings(config: dict[str, Any]) -> list[UnsupportedSetting]:
    ...
    for key, desired in config.get("unsupported", {}).items():
        reason = UNSUPPORTED_REASONS.get(
            key, "This setting is not applied by the v1 repository configurator."
        )
...
def build_ruleset_payload(
    ...
    if target == "branch" and ruleset_config.get("require_pull_request", False):
        ...
        if not allowed_merge_methods:
            raise ConfigError(
                "rulesets.branch_default.pull_request.allowed_merge_methods is empty "
                "and could not be derived from repository merge settings"
            )
        rules.append(
            {
                "type": "pull_request",
    ...
    payload = {
        "name": ruleset_config.get(
            "name", "Default branch" if target == "branch" else "Default tags"
        ),
        "target": target,
...
def resolve_bypass_actors(
    ...
    actors.sort(
        key=lambda item: (item["actor_type"], item.get("actor_id", -1), item["bypass_mode"])
    )
    return actors
...
def get_managed_rulesets(
    ...
    for entry in listed:
        ...
        if key in managed:
            raise ConfigError(
                f"Multiple repository rulesets found for managed rule {key[0]!r} ({key[1]})"
            )
...
def normalize_ruleset_payload(
    payload: dict[str, Any], managed_rule_types: set[str]
) -> dict[str, Any]:
    normalized_rules = []
    ...
    normalized_actors.sort(
        key=lambda actor: (actor["actor_type"], actor.get("actor_id", -1), actor["bypass_mode"])
    )
...
def normalize_rule(rule: dict[str, Any]) -> dict[str, Any]:
    ...
    if rule_type == "update":
        ...
        return {
            ...
            "parameters": {
                "update_allows_fetch_and_merge": bool(
                    params.get("update_allows_fetch_and_merge", False)
                )
            },
        }
...
def print_report(plan: PlanResult, applied: list[str] | None = None) -> None:
    ...
    if plan.unsupported:
        ...
        for item in plan.unsupported:
            print(
                f"- {item.key}: {item.reason} (desired={json.dumps(item.desired, sort_keys=True)})"
            )
